...
class PersonalInfoTable(QWidget):
    ...
    def initUI(self):
        ...
        headers = ['Họ và tên', 'Mã Sinh Viên', 'GPA', 'Giới tính', 'Ngày sinh','Lớp','Ngành']
        ...
        self.setWindowTitle('Danh sách thông tin cá nhân')
        self.show()
    ...
    def search_by_fullname(self):
        ...
        if fullname:
            filtered_data = [row for row in self.data if fullname.lower() in row['name'].lower()]
            ...
            for i, row in enumerate(filtered_data):
                ...
                self.table.setItem(i, 3, QTableWidgetItem(row['gender']))
                self.table.setItem(i, 4, QTableWidgetItem(row['date']))
                self.table.setItem(i, 5, QTableWidgetItem(row['lop']))
                self.table.setItem(i, 6, QTableWidgetItem(row['major']))
    ...
    def load_data(self):
        ...
        for i in range(6):
            self.table.setColumnWidth(i, int(len_table/7))
...
class PersonalInfo(QDialog):
    ...
    def save_info(self):
        ...
        if not name or not gender or not code or not gpa or not date or not lop or not major:
            QMessageBox.warning(self, 'Lỗi', 'Họ và tên, mã sinh viên, gpa, giới tính, ngày sinh, lớp, ngành là các trường bắt buộc')
            return
        ...
        self.parent().table.setItem(row_count, 3, QTableWidgetItem(gender))
        self.parent().table.setItem(row_count, 4, QTableWidgetItem(date))
        self.parent().table.setItem(row_count, 5, QTableWidgetItem(lop))
        self.parent().table.setItem(row_count, 6, QTableWidgetItem(major))

        # Thêm dữ liệu vào danh sách
        self.parent().data.append({"name":name,"code":code,"gpa":gpa, "gender":gender, "date":date, "lop":lop, "major":major})
...
class EditPersonalInfo(QDialog):
    ...
    def initUI(self):
        ...
        gender = self.parent().table.item(self.parent().current_row, 3).text()
        date = self.parent().table.item(self.parent().current_row, 4).text()
        lop = self.parent().table.item(self.parent().current_row, 5).text()
        major = self.parent().table.item(self.parent().current_row, 6).text()
    ...
    def save_info(self):
        ...
        self.parent().table.setItem(self.parent().current_row, 3, QTableWidgetItem(gender))
        self.parent().table.setItem(self.parent().current_row, 4, QTableWidgetItem(date))
        self.parent().table.setItem(self.parent().current_row, 5, QTableWidgetItem(lop))
        self.parent().table.setItem(self.parent().current_row, 6, QTableWidgetItem(major))

        # Cập nhật dữ liệu trong danh sách
        self.parent().data[self.parent().current_row] = {"name":name,"code":code,"gpa":gpa, "gender":gender, "date":date, "lop":lop, "major":major}
...
if __name__ == '__main__':
    # Dữ liệu mẫu
    data = [
        {'name': 'Nguyễn Văn C', 'code': 'B20DCCN123', 'gpa': '3.2', 'gender': 'Nam','date': '12/03/2002','lop': 'D20CQCN05','major': 'HTTT' },
        {'name': 'Nguyễn Văn A', 'code': 'B20DCCN123', 'gpa': '3.9', 'gender': 'Nam','date': '12/03/2002','lop': 'D20CQCN05','major': 'HTTT' },
        {'name': 'Nguyễn Văn B', 'code': 'B20DCCN123', 'gpa': '3.7', 'gender': 'Nam','date': '12/03/2002','lop': 'D20CQCN05','major': 'HTTT' },
        
    ]

    app = QApplication(sys.argv)
    table = PersonalInfoTable(data)
