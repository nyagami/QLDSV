...
class HocTapTable(QDialog):
    ...
    def initUI(self):
        ...
        headers = ['Mã sinh viên', 'Mã môn học', 'Điểm chuyên cần', 'Điểm kiểm tra','Điểm thực hành','Điểm thi']
    ...
    def search_by_fullname(self):
        ...
        if fullname:
            ...
            for i, row in enumerate(filtered_data):
                ...
                self.table.setItem(i, 1, QTableWidgetItem(str(row['code'])))
                # self.table.setItem(i, 2, QTableWidgetItem(str(row['gpa'])))
                self.table.setItem(i, 2, QTableWidgetItem(str(row['gender'])))
                self.table.setItem(i, 3, QTableWidgetItem(str(row['date'])))
                self.table.setItem(i, 4, QTableWidgetItem(str(row['lop'])))
                self.table.setItem(i, 5, QTableWidgetItem(str(row['major'])))
    ...
    def load_data(self):
        ...
        for i in range(5):
            self.table.setColumnWidth(i, int(len_table/6))
...
class EditPersonalInfo(QDialog):
    ...
    def save_info(self):
        ...
        self.parent().data[self.parent().current_row] = {"name":name,"code":code, "gender":gender, "date":date, "lop":lop, "major":major}
